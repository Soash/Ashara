...
from app1.models import Customer, GeneralAC, GeneralTransactionHistory, ShareAC, ShareACTransactionHistory
...
def ssm_deposit(request):
    ...
    data = SSM_Deposit.objects.filter()
...
def ssm_withdraw(request):
    ...
    data = SSM_Withdraw.objects.filter()
...
def get_out_loan(request):
    ...
    data = GetOutLoan.objects.filter().order_by('-id')
    
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        account_id = request.GET.get('account_id')
        try:
            out_loan = OutLoan.objects.get(pk=account_id,)
...
def profit_generate_monthly(request):
    if request.method == 'GET' and 'financial_year' in request.GET:
        ...
        total_share_accounts = ShareAC.objects.filter().count()

        if profit_type == 'General_Account_Daily':
            total_customers = Customer.objects.filter(customer_type='daily')
        elif profit_type == 'General_Account_Weekly':
            total_customers = Customer.objects.filter(customer_type='weekly')
        elif profit_type == 'General_Account_Monthly':
            total_customers = Customer.objects.filter(customer_type='monthly')
        ...
        random_customer = Customer.objects.filter().order_by('?').first()
        ...
        if profit_type == 'General_Account_Daily':
            total_customers = Customer.objects.filter(customer_type='daily', status='Active')
        elif profit_type == 'General_Account_Weekly':
            total_customers = Customer.objects.filter(customer_type='weekly', status='Active')
        elif profit_type == 'General_Account_Monthly':
            total_customers = Customer.objects.filter(customer_type='monthly', status='Active')
        elif profit_type == 'Share_Account':
            total_customers = Customer.objects.filter(status='Active')
        else:
            total_customers = []
...
def profit_distribution_history(request):
    profit_distributions = ProfitDistribution.objects.filter().order_by('-date')
    context = {
